...
sections:
  [
    ...
    { id: "usememo-vs-lazy-initializer", title: "useMemo vs Lazy Initializer" },
...
Já se deparou alguma vez em que você tinha uma situação como essa?
...
Uma coisa bem legal que pode ser feita para contornar esse caso é fazer um lazy initialization com o useState.
...
Basicamente se passarmos uma callback no useState, o hook fará com que essa função seja chamada apenas quando for necessário inicializar o estado inicial e quando o componente re-renderizar ele não vai executar novamente a função evitando assim que a expressão seja executada uma segunda vez.
...
Vale ressaltar que durante o re-render o garbage collector conseguiria desalocar a memória anterior antes de alocar um espaço novo na solução do **lazy initialization**, diferentemente da solução do useMemo ou useCallback, onde o garbage collector não desalocaria a memória e um novo espaço ainda precisa ser alocado, ou seja isso é pior no ponto de vista de perfomance em relação a memória.
